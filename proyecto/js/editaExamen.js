window.addEventListener("load", function () {
    ...
    function pedirPregunas() {
        ...
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 4 && ajax.status == 200) {
                ...
                if (respuesta.preguntas.length > 0) {
                    ...
                    j = 0;
                    while (j < respuesta.preguntas_exa[0].length) {

                        for (let i = 0; i < checkBox.length; i++) {
                            if (checkBox[i].parentNode.parentNode.id == respuesta.preguntas_exa[0][j].id) {
                                ...
                                j++;
                                if (j==respuesta.preguntas_exa[0].length) {
                                    i=checkBox.length;
                                }
                            }
                        }
        ...
        ajax.open("POST", "../php/pedirPreguntasEdit.php?exa=" + exam);
        ajax.send();
    }
    ...
    function EnviarEx() {
        ...
        ajax.open("POST", "../php/updateExamen.php");
        ajax.send(f);
